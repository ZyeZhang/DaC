...
import network
from data_list_mixmatch import ImageList, ImageList_twice
...
def train(args, txt_src, txt_tgt):
    ...
    ## set base network
    if args.net[0:3] == 'res':
        netG = network.ResBase(res_name=args.net)
    elif args.net[0:3] == 'vgg':
        netG = network.VGGBase(vgg_name=args.net)

    max_len = max(len(dset_loaders["source"]), len(dset_loaders["target"]))
    max_iter = args.max_epoch*max_len
    interval_iter = max_iter // 10

    netB = network.feat_bootleneck(type=args.classifier, feature_dim=netG.in_features, bottleneck_dim=args.bottleneck)
    netC = network.feat_classifier(type=args.layer, class_num = args.class_num, bottleneck_dim=args.bottleneck)
    ...
    if args.model == "source":
        modelpath = args.output_dir + "/source_F.pt" 
        netG.load_state_dict(torch.load(modelpath, map_location=device))
        modelpath = args.output_dir + "/source_B.pt"   
        netB.load_state_dict(torch.load(modelpath, map_location=device))
    else:    
        modelpath = args.output_dir + "/target_F_" + args.savename + ".pt" 
...
if __name__ == "__main__":
    ...
    parser.add_argument('--savename', type=str, default="par")
